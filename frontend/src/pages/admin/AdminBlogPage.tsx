...
export function AdminBlogPage() {
  ...
  const { data: posts = [] } = useQuery({
    queryKey: ['admin-blog'],
    queryFn: () => db.blogPosts.orderBy('createdAt').reverse().toArray(),
  });
  ...
  const openEdit = (post: BlogPost) => {
    setForm({
      ...
      body: post.body, image: post.image, author: post.author,
      category: post.category, readTime: post.readTime,
    });
    setEditing(post);
    setCreating(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (creating) {
        ...
        await db.blogPosts.update(editing.id, { ...form });
      }
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <Input
              ...
              placeholder="Education, Guides, Lifestyle..."
            />
            <Input
            ...
            <div className="md:col-span-2 flex flex-col gap-1.5">
              ...
              <p className="text-xs text-slate-400">Use ## for headings, **bold**, - for bullet lists</p>
              <textarea
                rows={16}
                className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y font-mono"
          ...
          <div className="flex gap-3">
            <Button onClick={handleSave} loading={saving}>
              <Save className="w-4 h-4" /> {creating ? 'Publish Post' : 'Save Changes'}
            </Button>
            ...
            {!creating && (
              <a
                ...
                target="_blank"
                className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-600 px-3 py-2"
              >
                ...
                <tr key={post.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      {post.image && (
                        <img src={post.image} alt="" className="w-10 h-10 rounded-lg object-cover shrink-0 bg-slate-100" />
                      )}
                      <div>
                  ...
                  <td className="px-4 py-4 hidden md:table-cell">
                    <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-full font-medium">{post.category}</span>
                  </td>
                      ...
                      {deleteId === post.id ? (
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className="text-xs text-slate-500 px-2 py-1 hover:bg-slate-100 rounded-lg"
                          >
                            Cancel
                          </button>
                        </div>
