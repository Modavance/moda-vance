      ...
      <div className="flex gap-1 border-b border-slate-100">
        {(['all', 'unread', 'read'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-2.5 text-sm font-semibold capitalize border-b-2 -mb-px transition-all ${filter === f ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
            ...
            <div className="divide-y divide-slate-50">
              {filtered.map(sub => (
                <div key={sub.id} onClick={() => handleOpen(sub)}
                  className={`px-5 py-4 cursor-pointer hover:bg-slate-50 transition-colors ${selected?.id === sub.id ? 'bg-blue-50' : ''}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${!sub.read ? 'bg-blue-600' : 'bg-transparent'}`} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className={`text-sm truncate ${!sub.read ? 'font-bold text-slate-900' : 'font-medium text-slate-700'}`}>{sub.name}</p>
              ...
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-1 text-sm text-slate-500">
                    <span className="flex items-center gap-1.5"><Mail className="w-4 h-4" /><a href={`mailto:${selected.email}`} className="text-blue-600 hover:underline">{selected.email}</a></span>
                    <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" />{formatDate(selected.createdAt)}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {deleteId === selected.id ? (
                    <>
                      <button onClick={() => handleDelete(selected.id)} className="text-xs text-red-600 font-semibold px-3 py-1.5 bg-red-50 rounded-lg hover:bg-red-100">Delete</button>
                      <button onClick={() => setDeleteId(null)} className="text-xs text-slate-500 px-2 py-1.5 hover:bg-slate-100 rounded-lg">Cancel</button>
                    </>
                  ) : (
                    <button onClick={() => setDeleteId(selected.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                  )}
                </div>
              </div>

              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl mb-6">
                <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-sm shrink-0">{selected.name[0].toUpperCase()}</div>
                <div>
                  <p className="font-semibold text-slate-900 text-sm">{selected.name}</p>
                  <p className="text-xs text-slate-500">{selected.email}</p>
                </div>
                {selected.read && <span className="ml-auto flex items-center gap-1 text-xs text-emerald-600 font-semibold"><CheckCircle className="w-3.5 h-3.5" /> Read</span>}
              </div>
