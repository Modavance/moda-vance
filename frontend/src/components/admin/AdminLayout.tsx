...
function NotificationBell() {
  ...
  const load = async () => {
    ...
    const [submissions, orders] = await Promise.all([
      db.contactSubmissions.orderBy('createdAt').reverse().limit(15).toArray(),
      db.orders.orderBy('createdAt').reverse().limit(15).toArray(),
    ]);
    ...
    const notifs: NotifItem[] = [
      ...
      ...orders.map(o => ({
        kind: 'order' as const,
        data: o,
        isNew: new Date(o.createdAt).getTime() > seenAt,
      })),
    ];
  ...
  const markAllRead = async () => {
    ...
    await Promise.all(unread.map(i => db.contactSubmissions.update(i.data.id, { read: true })));
    // Mark orders seen
  ...
  const handleClickItem = async (item: NotifItem) => {
    if (item.kind === 'message') {
      if (!(item.data as ContactSubmission).read) {
        await db.contactSubmissions.update(item.data.id, { read: true });
      }
      setOpen(false);
        ...
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <span className="font-semibold text-slate-900 text-sm">
              Notifications
              {totalUnread > 0 && (
                <span className="ml-2 px-1.5 py-0.5 bg-red-100 text-red-600 text-xs rounded-full font-medium">
                  {totalUnread} new
                </span>
              )}
            </span>
            ...
            {items.length === 0 ? (
              ...
              items.map((item, idx) => {
                const isUnread = item.kind === 'message'
                  ? !(item.data as ContactSubmission).read
                  : item.isNew;
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between gap-2">
                          <p className="text-sm font-medium text-slate-900 truncate">
                            {item.kind === 'message'
                              ? (item.data as ContactSubmission).name
                              : `New order ${item.data.id.slice(0, 14)}…`
                            }
                          </p>
                        ...
                        <p className="text-xs text-slate-500 truncate">
                          {item.kind === 'message'
                            ? (item.data as ContactSubmission).subject
                            : `$${(item.data as Order).total.toFixed(2)} · ${(item.data as Order).status}`
                          }
                        </p>
                      </div>
          ...
          <div className="px-4 py-3 border-t border-slate-100 flex gap-3">
            <Link
              ...
              className="flex-1 text-center text-sm text-violet-600 hover:text-violet-700 font-medium"
            >
              Messages →
            </Link>
            ...
            <Link
              to="/admin/orders"
              onClick={() => setOpen(false)}
              className="flex-1 text-center text-sm text-emerald-600 hover:text-emerald-700 font-medium"
            >
              Orders →
            </Link>
          </div>
...
function ProfileMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(v => !v)}
        className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-sm font-bold hover:bg-blue-700 transition-colors ring-2 ring-transparent hover:ring-blue-200"
      >
        A
      </button>
      ...
      <div className="px-6 py-5 border-b border-slate-800">
        <div className="flex items-center gap-2">
          <svg width="32" height="32" viewBox="0 0 40 40" fill="none">
            ...
            <defs>
              <linearGradient id="ag" x1="3.5" y1="2" x2="36.5" y2="38" gradientUnits="userSpaceOnUse">
                <stop stopColor="#3b82f6"/><stop offset="1" stopColor="#2563eb"/>
              </linearGradient>
            </defs>
          </svg>
          <div>
            <p className="text-white font-bold text-sm">ModaVance</p>
            <p className="text-slate-500 text-xs uppercase tracking-wider">Admin</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon className={cn('w-4.5 h-4.5', isActive ? 'text-white' : 'text-slate-400 group-hover:text-white')} />
                {item.label}
                {!isActive && <ChevronRight className="w-3.5 h-3.5 ml-auto opacity-0 group-hover:opacity-50 transition-opacity" />}
              </>
            )}
          </NavLink>
        ))}
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {sidebarOpen && (
        <>
          ...
          <aside className="fixed left-0 top-0 h-full w-60 z-50 lg:hidden">
            <Sidebar mobile />
          </aside>
        </>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between shrink-0">
          ...
          <div className="hidden lg:block">
            <p className="text-sm text-slate-400">Good morning, Admin</p>
          </div>
