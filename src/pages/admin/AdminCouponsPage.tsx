...
function AddCouponModal({ onClose, onSave }: { onClose: () => void; onSave: (c: Coupon) => Promise<void> }) {
  const [form, setForm] = useState({
    code: '', discount: 10, type: 'percent' as 'percent' | 'fixed',
    minOrder: 0, expiresAt: '2027-12-31',
  });
...
export function AdminCouponsPage() {
  ...
  const { data: coupons } = useQuery({
    queryKey: ['coupons'],
    queryFn: () => db.coupons.toArray(),
  });

  const handleAdd = async (coupon: Coupon) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{coupons?.length ?? 0} active coupons</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          return (
            <div key={coupon.code} className={`bg-white rounded-2xl border p-5 relative overflow-hidden ${isExpired ? 'border-slate-200 opacity-60' : 'border-slate-100'}`}>
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => handleDelete(coupon.code)} className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
              ...
              <div className="text-center py-4 border-t border-b border-dashed border-slate-200 mb-4">
                ...
                <p className="text-sm text-slate-500 font-medium mt-0.5">
                  {coupon.type === 'percent' ? 'off total order' : 'off your order'}
                </p>
              </div>
        ...
        <button onClick={() => setShowAdd(true)}
          ...
          <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center group-hover:border-blue-300">
